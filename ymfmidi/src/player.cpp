...
// ----------------------------------------------------------------------------
void OPLPlayer::setSequence(Sequence* sequence)
{
	delete m_sequence;
	m_sequence = sequence;
}
...
{
    // DMX volume curve
    static const uint8_t opl_volume_map[128] = {
	...
	uint8_t atten = 0x3f ^ ((opl_volume_map[voice.channel->volume] * opl_volume_map[voice.velocity]) >> 9);
	uint8_t level;
