...
//-----------------------------------------------------------------------------
//
// $Id: p_saveg.c,v 1.27 2003/08/11 13:50:01 hurdler Exp $
//
...
void P_UnArchivePlayers(void)
{
    ...
    for (i = 0; i < MAXPLAYERS; i++)
    {
        ...
        players[i].readyweapon = (weapontype_t)READBYTE(save_p);
        players[i].pendingweapon = (weapontype_t)READBYTE(save_p);
        players[i].playerstate = (playerstate_t)READBYTE(save_p);
...
void P_ArchiveWorld(void)
{
    ...
    for (i = 0; i < numsectors; i++, ss++, ms++)
    {
        diff = 0;
        diff2 = 0;
        if (ss->floorheight != LE_SWAP_SHORT(ms->floorheight) << FRACBITS)
            diff |= SD_FLOORHT;
        if (ss->ceilingheight != LE_SWAP_SHORT(ms->ceilingheight) << FRACBITS)
            diff |= SD_CEILHT;
        //
        //  flats
        //
        ...
        if (ss->lightlevel != LE_SWAP_SHORT(ms->lightlevel))
            diff |= SD_LIGHT;
        if (ss->special != LE_SWAP_SHORT(ms->special))
            diff |= SD_SPECIAL;
    ...
    for (i = 0; i < numlines; i++, mld++, li++)
    {
        ...
        if (((cv_deathmatch.value == 0) && (li->flags != LE_SWAP_SHORT(mld->flags))) || ((cv_deathmatch.value != 0) && ((li->flags & ~ML_MAPPED) != LE_SWAP_SHORT(mld->flags))))
            diff |= LD_FLAG;
        if (li->special != LE_SWAP_SHORT(mld->special))
            diff |= LD_SPECIAL;

        if (li->sidenum[0] != -1)
        {
            si = &sides[li->sidenum[0]];
            if (si->textureoffset != LE_SWAP_SHORT(msd[li->sidenum[0]].textureoffset) << FRACBITS)
                diff |= LD_S1TEXOFF;
        ...
        if (li->sidenum[1] != -1)
        {
            ...
            if (si->textureoffset != LE_SWAP_SHORT(msd[li->sidenum[1]].textureoffset) << FRACBITS)
                diff2 |= LD_S2TEXOFF;
    ...
    while (1)
    {
        ...
        switch (tclass)
        {
            case tc_mobj:
                ...
                mobj = (mobj_t *)Z_Malloc(sizeof(mobj_t), PU_LEVEL, NULL);
                ...
                if (diff & MD_TYPE)
                {
                    mobj->type = (mobjtype_t)READULONG(save_p);
                }
                ...
                if (diff & MD_SPRITE)
                    mobj->sprite = (spritenum_t)READUSHORT(save_p);
                else
            ...
            case tc_ceiling:
                ...
                save_p += sizeof(*ceiling);
                ceiling->sector = &sectors[LE_SWAP_LONG((long)ceiling->sector)];
            ...
            case tc_door:
                ...
                save_p += sizeof(*door);
                door->sector = &sectors[LE_SWAP_LONG((long)door->sector)];
                door->sector->ceilingdata = door;
                door->line = &lines[LE_SWAP_LONG((long)door->line)];
            ...
            case tc_floor:
                ...
                save_p += sizeof(*floor);
                floor->sector = &sectors[LE_SWAP_LONG((long)floor->sector)];
            ...
            case tc_plat:
                ...
                save_p += sizeof(*plat);
                plat->sector = &sectors[LE_SWAP_LONG((long)plat->sector)];
            ...
            case tc_flash:
                ...
                save_p += sizeof(*flash);
                flash->sector = &sectors[LE_SWAP_LONG((long)flash->sector)];
            ...
            case tc_strobe:
                ...
                save_p += sizeof(*strobe);
                strobe->sector = &sectors[LE_SWAP_LONG((long)strobe->sector)];
            ...
            case tc_glow:
                ...
                save_p += sizeof(*glow);
                glow->sector = &sectors[LE_SWAP_LONG((long)glow->sector)];
            ...
            case tc_fireflicker:
                ...
                save_p += sizeof(*fireflicker);
                fireflicker->sector = &sectors[LE_SWAP_LONG((long)fireflicker->sector)];
            ...
            case tc_elevator:
                ...
                elevator->sector = &sectors[LE_SWAP_LONG((long)elevator->sector)];
...
void P_UnArchiveLevelScript()
{
    ...
    for (i = 0; i < num_variables; i++)
    {
        ...
        // name
        sv->name = Z_Strdup((const char*)save_p, PU_LEVEL, 0);
        ...
        switch (sv->type)       // read depending on type
        {
            case svt_string:
            {
                sv->value.s = Z_Strdup((const char*)save_p, PU_LEVEL, 0);
                save_p += strlen(sv->value.s) + 1;
                break;
            }
            case svt_int:
...
runningscript_t *P_UnArchiveRunningScript()
{
    ...
    rs->wait_type = (waitType_t)READSHORT(save_p);
...
bool P_UnArchiveMisc()
{
    ...
    gameskill = (skill_t)READBYTE(save_p);
