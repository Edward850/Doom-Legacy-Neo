...
//-----------------------------------------------------------------------------
//
// $Id: p_spec.c,v 1.45 2003/07/23 17:26:36 darkwolf95 Exp $
//
...
void P_InitPicAnims (void)
{
  ...
  for (i = 0; animdefs[i].istexture != -1; i++)
  {
    ...
    lastanim->speed = LE_SWAP_LONG(animdefs[i].speed);
    lastanim++;
  }
  ...
  mobj_t*       thing)
{
    ...
    switch (line->special)
    {
      ...
      case 8:
        // Build Stairs
        if(EV_BuildStairs(line,(gamemode == heretic) ? (stair_e)(8*FRACUNIT) : build8) || !boomsupport)
          line->special = 0;
        break;
...
void P_ProcessSpecialSector(player_t* player, sector_t* sector, bool instantdamage)
{
  ...
  for(rover = sector->ffloors; rover; rover = rover->next)
  {
    ...
    if(rover->flags & FF_SOLID)
    {
      ...
      if (demoversion >= 125 &&
        ...
        sector->heightsec == -1 && (leveltime % 2)) //SoM: penalize jumping less.
        instantdamage = true;
      else
...
void P_AddFakeFloor(sector_t* sec, sector_t* sec2, line_t* master, int flags)
{
  ...
  //Add the floor
  ffloor = (ffloor_t *)Z_Malloc(sizeof(ffloor_t), PU_LEVEL, NULL);
