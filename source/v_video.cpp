...
//-----------------------------------------------------------------------------
//
// $Id: v_video.c,v 1.34 2003/08/11 13:50:00 hurdler Exp $
//
...
static void LoadPalette(char *lumpname)
{
    ...
    pal = (byte*)W_CacheLumpNum(i, PU_CACHE);
    for (i = 0; i < palsize; i++)
    {
...
void V_DrawMappedPatch(int x, int y, int scrn, patch_t * patch, byte * colormap)
{
    ...
    y -= LE_SWAP_SHORT(patch->topoffset);
    x -= LE_SWAP_SHORT(patch->leftoffset);
    ...
    if (!scrn)
        V_MarkRect(x, y, LE_SWAP_SHORT(patch->width) * dupx, LE_SWAP_SHORT(patch->height) * dupy);

    col = 0;
    colfrac = FixedDiv(FRACUNIT, dupx << FRACBITS);
    rowfrac = FixedDiv(FRACUNIT, dupy << FRACBITS);

    w = LE_SWAP_SHORT(patch->width) << FRACBITS;

    for (; col < w; col += colfrac, desttop++)
    {
        column = (column_t *) ((byte *) patch + LE_SWAP_LONG(patch->columnofs[col >> FRACBITS]));
                       ...
                       patch_t * patch)
{
    ...
    destend = desttop + LE_SWAP_SHORT(patch->width) * dupx;

    if (scrn & V_FLIPPEDPATCH)
    {
        colfrac = -colfrac;
        col = (LE_SWAP_SHORT(patch->width) << FRACBITS) + colfrac;
    }
    else
        col = 0;
...
//    if( (scrn & V_NOSCALEPATCH) )
    ...
    {
        desttop += (y * vid.width) + x;
        destend = desttop + LE_SWAP_SHORT(patch->width) / 2 * dupx;
    }
    else
    {
        desttop += (y * vid.width) + x;
                            ...
                            patch_t * patch)
{
    ...
    y -= LE_SWAP_SHORT(patch->topoffset) * dupy;
    x -= LE_SWAP_SHORT(patch->leftoffset) * dupx;
...
void V_DrawPatch(int x, int y, int scrn, patch_t * patch)
{
    ...
    x -= LE_SWAP_SHORT(patch->leftoffset);
#ifdef RANGECHECK
    if (x < 0 || x + LE_SWAP_SHORT(patch->width) > vid.width || y < 0 || y + LE_SWAP_SHORT(patch->height) > vid.height || (unsigned) scrn > 4)
    {
    ...
    if (!scrn)
        V_MarkRect(x, y, LE_SWAP_SHORT(patch->width), LE_SWAP_SHORT(patch->height));

    col = 0;
    desttop = screens[scrn] + y * vid.width + x;

    w = LE_SWAP_SHORT(patch->width);

    for (; col < w; x++, col++, desttop++)
    {
        column = (column_t *) ((byte *) patch + LE_SWAP_LONG(patch->columnofs[col]));
                    ...
                    int lumpnum)
{
    ...
    V_BlitScalePic(x1, y1, scrn, (pic_t*)W_CacheLumpNum(lumpnum, PU_CACHE));
}
...
static void V_BlitScalePic(int x1, int y1, int scrn, pic_t * pic)
{
    ...
    width = LE_SWAP_SHORT(pic->width);
    height = LE_SWAP_SHORT(pic->height);
    scrn &= 0xffff;
...
void V_DrawFlatFill(int x, int y, int w, int h, int flatnum)
{
    ...
    flat = (byte*)W_CacheLumpNum(flatnum, PU_CACHE);
...
void V_DrawString(int x, int y, int option, char *string)
{
    ...
    while (1)
    {
        ...
        w = LE_SWAP_SHORT(hu_font[c]->width) * dupx;
        if (cx + w > scrwidth)
            break;
...
int V_StringWidth(char *string)
{
    ...
    for (i = 0; i < (int) strlen(string); i++)
    {
        ...
        if (c < 0 || c >= HU_FONTSIZE)
            ...
            w += LE_SWAP_SHORT(hu_font[c]->width);
    }
