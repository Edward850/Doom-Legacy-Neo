...
static bool bNetworkInit = false; // Has the networking subsystem been initialized?
...
static void DTLSSendPacket(socket_t* dtls_socket, const byte* data, size_t length)
{
...
static void DTLSConfigureMasterServerCertificate(const char* certAddress)
{
    ...
    char* filePath = (char*)Z_Malloc(path_length, PU_STATIC, NULL);
    char* serverPath = (char*)Z_Malloc(serverPath_length, PU_STATIC, NULL);
