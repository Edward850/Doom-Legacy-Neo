...
//-----------------------------------------------------------------------------
//
// $Id: r_things.c,v 1.44 2002/11/12 00:06:05 ssntails Exp $
//
...
extern double adjustedlightscale;
...
//
bool R_AddSingleSpriteDef(char* sprname, spritedef_t* spritedef, int wadnum, int startlump, int endlump)
{
    int         l;
    int         intname;
    int         frame;
    ...
    for (l = startlump; l < endlump; l++)
    {
        if (*(int*)lumpinfo[l].name == intname)
        {
            ...
            spritewidth[numspritelumps] = LE_SWAP_SHORT(patch.width) << FRACBITS;
            spriteoffset[numspritelumps] = LE_SWAP_SHORT(patch.leftoffset) << FRACBITS;
            spritetopoffset[numspritelumps] = LE_SWAP_SHORT(patch.topoffset) << FRACBITS;
            spriteheight[numspritelumps] = LE_SWAP_SHORT(patch.height) << FRACBITS;

#ifdef HWRENDER
            //BP: we cannot use special tric in hardware mode because feet in ground caused by z-buffer
            if (rendermode != render_soft && LE_SWAP_SHORT(patch.topoffset) > 0 // not for psprite
                && LE_SWAP_SHORT(patch.topoffset) < LE_SWAP_SHORT(patch.height))
                // perfect is patch.height but sometime it is too high
                spritetopoffset[numspritelumps] = min(LE_SWAP_SHORT(patch.topoffset + 4), LE_SWAP_SHORT(patch.height)) << FRACBITS;
...
#ifdef RANGECHECK
        if (texturecolumn < 0 || texturecolumn >= LE_SWAP_SHORT(patch->width))
            I_Error("R_DrawSpriteRange: bad texturecolumn");
#endif
        column = (column_t*)((byte*)patch +
            LE_SWAP_LONG(patch->columnofs[texturecolumn]));
    ...
    // adjust the heights.
        ...
        if (!(sector->lightlist[i].caster->flags & FF_NOSHADE))
        {
            ...
            else
            {
                ...
                {
                    fixed_t lightScale = adjustedlightscale * FRACUNIT;
                    lightScale = FixedMul(sprite->xscale, lightScale);
                    index = lightScale >> (LIGHTSCALESHIFT - detailshift);
...
static void R_ProjectSprite(mobj_t* thing)
{
    ...
    if (thing->frame & FF_SMOKESHADE)
        ...
        vis->colormap = (lighttable_t*)VIS_SMOKESHADE;
    else
    {
        if (thing->frame & FF_TRANSMASK)
        ...
        {
            fixed_t lightScale = adjustedlightscale * FRACUNIT;
            lightScale = FixedMul(xscale, lightScale);
            index = lightScale >> (LIGHTSCALESHIFT - detailshift);
...
void R_AddSkins(int wadnum)
{
    ...
    while ((lumpnum = W_CheckForSkinMarkerInPwad(wadnum, lastlump)) != -1)
    {
        ...
        buf = (char *)W_CacheLumpNum(lumpnum, PU_CACHE);
