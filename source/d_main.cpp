...
//-----------------------------------------------------------------------------
//
// $Id: d_main.c,v 1.63 2003/11/21 17:52:05 darkwolf95 Exp $
//
...
bool infight = false;		//DarkWolf95:November 21, 2003: Monsters Infight!

bool singletics = false;     // timedemo

bool nomusic;
bool nosound;
bool digmusic;               // OGG/MP3 Music SSNTails 12-13-2002
...
void D_Display(void)
{
    ...
    static gamestate_t oldgamestate = GS_FORCED;
    ...
    if (setsizeneeded)
    {
        ...
        oldgamestate = GS_FORCED;      // force background redraw
        borderdrawcount = 3;
        redrawsbar = true;
    }
...
//
    ...
    do
    {
        ...
        I_FinishUpdate();       // page flip or blit buffer
        I_UpdateSound(players[displayplayer].mo);
    }
...
//    char*       doom2fwad;
    ...
    // and... Doom LEGACY !!! :)
    legacywad = (char *)malloc(strlen(doomwaddir) + 1 + 10 + 1);
    ...
    hereticwad = (char *)malloc(strlen(doomwaddir) + 1 + 12 + 1);
    sprintf(hereticwad, "%s/heretic.wad", doomwaddir);

    heretic1wad = (char *)malloc(strlen(doomwaddir) + 1 + 12 + 1);
    sprintf(heretic1wad, "%s/heretic1.wad", doomwaddir);

    hexenwad = (char *)malloc(strlen(doomwaddir) + 1 + 12 + 1);
    sprintf(hexenwad, "%s/hexen.wad", doomwaddir);

	//DarkWolf95:July 14, 2003:Chex Quest Support
	chex1wad = (char *)malloc(strlen(doomwaddir)+1+12+1);
    ...
    // will be overwrite in case of -cdrom or linux home
    {
        ...
        strConfigfile = (char *)malloc(strConfigfileSize);
...
*/
    ...
    {
        int result = 0;
        auto [ptr, ec] = std::from_chars(myargv[p + 1], myargv[p + 1] + strlen(myargv[p + 1]), result);
        if (ec != std::errc())
        {
            CONS_Printf("Invalid skill number: %s\n", myargv[p + 1]);
        }
        else if (result < 0 || result > 4)
        {
            CONS_Printf("Skill number must be between 0 and 4: %d\n", result);
        }
        else
        {
            gameskill = (skill_t)result;
        }
        autostart = true;
    }
