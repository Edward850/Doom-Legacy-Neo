...
//-----------------------------------------------------------------------------
//
// $Id: st_stuff.c,v 1.22 2003/08/11 13:50:00 hurdler Exp $
//
...
void ST_loadFaceGraphics (char *facestr)
{
    ...
    if( i!=-1 )
        faceback = W_CachePatchNum(i, PU_STATIC);
    else
...
void ST_Init (void)
{
    ...
    scr_borderpatch = (byte*)W_CacheLumpNum (st_borderpatchnum, PU_STATIC);
    if( gamemode == heretic )
    {
        SB_Init();
        return;
