...
// NET GAME STUFF
#define NG_STATSY               50
#define NG_STATSX               (32 + LE_SWAP_SHORT(star->width)/2 + 32*!dofrags)
...
static void WI_drawLF(void)
{
    ...
    {
        V_DrawScaledPatch ((BASEVIDWIDTH - LE_SWAP_SHORT(lnames[wbs->last]->width))/2,
                            y, FB, lnames[wbs->last]);
        y += (5 * LE_SWAP_SHORT(lnames[wbs->last]->height))/4;
        // draw "Finished!"
        V_DrawScaledPatch ((BASEVIDWIDTH - LE_SWAP_SHORT(finished->width))/2,
                            y, FB, finished);
    }
}
...
static void WI_drawEL(void)
{
    ...
    {
        V_DrawScaledPatch((BASEVIDWIDTH - LE_SWAP_SHORT(entering->width))/2,
                          y, FB, entering);

        // draw level
        y += (5 * LE_SWAP_SHORT(lnames[wbs->next]->height))/4;

        V_DrawScaledPatch((BASEVIDWIDTH - LE_SWAP_SHORT(lnames[wbs->next]->width))/2,
                             ...
                             patch_t*      c[] )
{
    ...
    do
    {
        left   = lnodes->x - LE_SWAP_SHORT(c[i]->leftoffset);
        top    = lnodes->y - LE_SWAP_SHORT(c[i]->topoffset);
        right  = left + LE_SWAP_SHORT(c[i]->width);
        bottom =  top + LE_SWAP_SHORT(c[i]->height);
                        ...
                        int           digits )
{
    ...
    int         fontwidth = LE_SWAP_SHORT(num[0]->width);
    int         neg;
    int         temp;
                          ...
                          int           t )
{
    ...
    if (t <= 61*59)
    {
        ...
        {
            n = (t / div) % 60;
            x = WI_drawNum(x, y, n, 2) - LE_SWAP_SHORT(colon->width);
            div *= 60;
    ...
    {
        // "sucks"
        V_DrawScaledPatch(x - LE_SWAP_SHORT(sucks->width), y, FB, sucks);
    }
}
...
static void WI_ddrawDeathmatchStats(void)
{
    ...
    V_DrawScaledPatch(DM_TOTALSX-LE_SWAP_SHORT(total->width)/2,
    ...
    for (i=0 ; i<MAXPLAYERS ; i++)
    {
        if (playeringame[i])
        {
            ...
            V_DrawMappedPatch(x-LE_SWAP_SHORT(stpb->width)/2,
                        DM_MATRIXY - WI_SPACINGY,
                        FB,
                        stpb,      //p[i], now uses a common STPB0 translated
                        colormap); //      to the right colors

            V_DrawMappedPatch(DM_MATRIXX-LE_SWAP_SHORT(stpb->width)/2,
                        y,
                        FB,
                        stpb,      //p[i]
                        colormap);
            ...
            if (i == me)
            {
                V_DrawScaledPatch(x-LE_SWAP_SHORT(stpb->width)/2,
                            DM_MATRIXY - WI_SPACINGY,
                            FB,
                            bstar);

                V_DrawScaledPatch(DM_MATRIXX-LE_SWAP_SHORT(stpb->width)/2,
                            y,
                            FB,
                            star);
            }
        ...
        {
            // V_DrawPatch(x-LE_SWAP_SHORT(bp[i]->width)/2,
            //   DM_MATRIXY - WI_SPACINGY, FB, bp[i]);
            // V_DrawPatch(DM_MATRIXX-LE_SWAP_SHORT(bp[i]->width)/2,
            //   y, FB, bp[i]);
        }
        x += DM_SPACINGX;
        y += WI_SPACINGY;
...
static void WI_drawNetgameStats(void)
{
    ...
    int         x;
    int         y;
    int         pwidth = LE_SWAP_SHORT(percent->width);
    ...
    {
        V_DrawScaledPatch(NG_STATSX+NG_SPACINGX- LE_SWAP_SHORT(kills->width),
            NG_STATSY, FB, kills);
        
        V_DrawScaledPatch(NG_STATSX+2*NG_SPACINGX- LE_SWAP_SHORT(items->width),
            NG_STATSY, FB, items);
        
        V_DrawScaledPatch(NG_STATSX+3*NG_SPACINGX- LE_SWAP_SHORT(secret->width),
            NG_STATSY, FB, secret);
        if (dofrags)
            V_DrawScaledPatch(NG_STATSX+4*NG_SPACINGX- LE_SWAP_SHORT(frags->width),
                              NG_STATSY, FB, frags);
        // draw stats
        y = NG_STATSY + LE_SWAP_SHORT(kills->height);
    }
    ...
    for (i=0 ; i<MAXPLAYERS ; i++)
    {
        ...
        V_DrawMappedPatch(x-LE_SWAP_SHORT(stpb->width), y, FB, stpb, colormap);

        if (i == me)
            V_DrawScaledPatch(x-LE_SWAP_SHORT(stpb->width), y, FB, star);
...
static void WI_drawStats(void)
{
    ...
    lh = (3 * LE_SWAP_SHORT(num[0]->height))/2;
