...
//-----------------------------------------------------------------------------
//
// $Id: d_net.c,v 1.18 2003/05/04 02:29:14 sburke Exp $
//
...
static bool GetFreeAcknum(byte *freeack, bool lowtimer)
{
...
static bool inline Processackpak()
{
   int i;
...
extern void Net_AckTicker(void)
{
    ...
    for(i=0;i<MAXACKPACKETS;i++)
    {
        netNode_t *node=&nodes[ackpak[i].destinationnode];
        if(ackpak[i].acknum)
...
extern bool Net_AllAckReceived(void)
{
   int i;
...
{
    int i;
    bool forceclose = (node & FORCECLOSE)!=0;
    node &= ~FORCECLOSE;
...
static void DebugPrintpacket(char *header)
{
    ...
    switch(netbuffer->packettype)
    {
       ...
       case PT_SERVERINFO :
           ...
           fprintfstring((char *)&netbuffer->u.serverinfo.fileneeded[0], (char*)netbuffer + doomcom->datalength - (char*)netbuffer->u.serverinfo.fileneeded);
           break;
       ...
       default : // write as a raw packet
           fprintfstring((char *)&netbuffer->u.textcmd[0], (char *)netbuffer+doomcom->datalength-(char *)&netbuffer->u.textcmd[0]);
           break;
...
// HSendPacket
//
extern bool HSendPacket(int   node,bool reliable ,byte acknum,int packetlength)
{
...
//
extern bool D_CheckNetGame (void)
{
    bool ret=false;
    ...
    if( !netgame )
    {
        doomcom = (doomcom_t *)Z_Malloc(sizeof(doomcom_t),PU_STATIC,NULL);
