...
//
bool         newlevel = false;
bool         doom1level = false;    // doom 1 level running under doom 2
...
void P_LoadVertexes (int lump)
{
    ...
    {
        li->x = LE_SWAP_SHORT(ml->x)<<FRACBITS;
        li->y = LE_SWAP_SHORT(ml->y)<<FRACBITS;
    }
...
void P_LoadSegs (int lump)
{
    ...
    {
        li->v1 = &vertexes[LE_SWAP_SHORT(ml->v1)];
        li->v2 = &vertexes[LE_SWAP_SHORT(ml->v2)];
        ...
        li->angle = (LE_SWAP_SHORT(ml->angle))<<16;
        li->offset = (LE_SWAP_SHORT(ml->offset))<<16;
        linedef = LE_SWAP_SHORT(ml->linedef);
        ldef = &lines[linedef];
        li->linedef = ldef;
        li->side = side = LE_SWAP_SHORT(ml->side);
...
void P_LoadSubsectors (int lump)
{
    ...
    {
        ss->numlines = LE_SWAP_SHORT(ms->numsegs);
        ss->firstline = LE_SWAP_SHORT(ms->firstseg);
    }
...
void P_LoadSectors (int lump)
{
    ...
    {
        ss->floorheight = LE_SWAP_SHORT(ms->floorheight)<<FRACBITS;
        ss->ceilingheight = LE_SWAP_SHORT(ms->ceilingheight)<<FRACBITS;
        ...
        ss->lightlevel = LE_SWAP_SHORT(ms->lightlevel);
        ss->special = LE_SWAP_SHORT(ms->special);
        ss->tag = LE_SWAP_SHORT(ms->tag);
...
void P_LoadNodes (int lump)
{
    ...
    {
        no->x = LE_SWAP_SHORT(mn->x)<<FRACBITS;
        no->y = LE_SWAP_SHORT(mn->y)<<FRACBITS;
        no->dx = LE_SWAP_SHORT(mn->dx)<<FRACBITS;
        no->dy = LE_SWAP_SHORT(mn->dy)<<FRACBITS;
        for (j=0 ; j<2 ; j++)
        {
            no->children[j] = LE_SWAP_SHORT(mn->children[j]);
            for (k=0 ; k<4 ; k++)
                no->bbox[j][k] = LE_SWAP_SHORT(mn->bbox[j][k])<<FRACBITS;
        }
    }
...
void P_LoadThings (int lump)
{
    ...
    data = datastart = (char *)W_CacheLumpNum (lump,PU_LEVEL);
...
void P_LoadLineDefs (int lump)
{
    ...
    {
        ld->flags = LE_SWAP_SHORT(mld->flags);
        ld->special = LE_SWAP_SHORT(mld->special);
        ld->tag = LE_SWAP_SHORT(mld->tag);
        v1 = ld->v1 = &vertexes[LE_SWAP_SHORT(mld->v1)];
        v2 = ld->v2 = &vertexes[LE_SWAP_SHORT(mld->v2)];
        ld->dx = v2->x - v1->x;
        ld->dy = v2->y - v1->y;
        ...
        ld->sidenum[0] = LE_SWAP_SHORT(mld->sidenum[0]);
        ld->sidenum[1] = LE_SWAP_SHORT(mld->sidenum[1]);
...
/*void P_LoadSideDefs (int lump)
{
    ...
    {
        sd->textureoffset = LE_SWAP_SHORT(msd->textureoffset)<<FRACBITS;
        sd->rowoffset = LE_SWAP_SHORT(msd->rowoffset)<<FRACBITS;
        sd->toptexture = R_TextureNumForName(msd->toptexture);
        sd->bottomtexture = R_TextureNumForName(msd->bottomtexture);
        sd->midtexture = R_TextureNumForName(msd->midtexture);

        sd->sector = &sectors[LE_SWAP_SHORT(msd->sector)];
    }
  ...
  for (i=0; i<numsides; i++)
    {
      ...
      sd->sector = sec = &sectors[LE_SWAP_SHORT(msd->sector)];
      switch (sd->special)
        {
  ...
  count = W_LumpLength(lump)/2;
    {
      ...
      blockmaplump = (long *)Z_Malloc(sizeof(*blockmaplump) * count, PU_LEVEL, 0);
      ...
      blockmaplump[0] = LE_SWAP_SHORT(wadblockmaplump[0]);
      blockmaplump[1] = LE_SWAP_SHORT(wadblockmaplump[1]);
      blockmaplump[2] = (long)(LE_SWAP_SHORT(wadblockmaplump[2])) & 0xffff;
      blockmaplump[3] = (long)(LE_SWAP_SHORT(wadblockmaplump[3])) & 0xffff;

      for (i=4 ; i<count ; i++)
        {
          short t = LE_SWAP_SHORT(wadblockmaplump[i]);          // killough 3/1/98
...
/* Original
		...
		for (i=0 ; i<count ; i++)
			blockmaplump[i] = LE_SWAP_SHORT(blockmaplump[i]);
...
void P_GroupLines (void)
{
    ...
    linebuffer = (line_t **)Z_Malloc((int)((size_t)total * sizeof(*linebuffer)), PU_LEVEL, 0);
    sector = sectors;
    for (i=0 ; i<numsectors ; i++, sector++)
    {
		...
		memset(&sector->soundorg, 0, sizeof(sector->soundorg));
...
//    R_FlushTextureCache();
    ...
    rejectmatrix = (byte *)W_CacheLumpNum (lastloadedmaplumpnum+ML_REJECT,PU_LEVEL);
