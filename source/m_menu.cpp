...
typedef struct menu_s
{
    ...
    bool         (*quitroutine)(void);   // called before quit a menu return true if we can
} menu_t;
...
void M_ClearMenus (bool callexitmenufunc);
...
static bool StartSplitScreenGame;
...
bool M_QuitMultiPlayerMenu(void);
...
{
    int      l;
    bool  exitmenu = false;  // exit to previous menu and send name change
    int      myskin;
...
void M_ChooseSkill(int choice)
{
    ...
    G_DeferedInitNew((skill_t)choice, G_BuildMapName(epi+1,1),StartSplitScreenGame);
    M_ClearMenus (true);
}
...
void M_VerifyNightmare(int ch)
{
    ...
    G_DeferedInitNew ((skill_t)nightmare, G_BuildMapName(epi+1,1),StartSplitScreenGame);
    M_ClearMenus (true);
}
...
//
static  bool setupcontrols_secondaryplayer;
...
void M_ReadSaveStrings(void)
{
    ...
    finalPath = (char *)Z_Malloc(strlen(initalPath) + 20, PU_STATIC, 0);
                    ...
                    consvar_t *cv)
{
    ...
    xx += LE_SWAP_SHORT(p->width)-LE_SWAP_SHORT(p->leftoffset);
    for (i=0;i<16;i++)
    {
...
void M_DrawMessageMenu(void)
{
    ...
    while(*(msg+start))
    {
        ...
        y += 8; //LE_SWAP_SHORT(hu_font[0]->height);
    }
}
...
bool M_ChangeStringCvar(int choise)
{
    consvar_t *cv=(consvar_t *)currentMenu->menuitems[itemOn].itemaction;
    char buf[255];
    int  len;
    ...
    if(currentMenu->menuitems[itemOn].status==IT_MSGHANDLER)
    {
        ...
        else
        {
            ...
            if (ev->type==ev_mouse || ev->type==ev_joystick )
                return true;
            if(routine == (void (*)(int))M_ChangecontrolResponse)
