...
//-----------------------------------------------------------------------------
//
// $Id: hw_draw.c,v 1.26 2004/04/18 20:40:31 hurdler Exp $
//
...
//        return;
    ...
    if (clearlines > basewindowy-8) {
        patch = (GlidePatch_t*)W_CachePatchNum (viewborderlump[BRDR_T],PU_CACHE);
    ...
    if (clearlines > basewindowy+baseviewheight) {
        patch = (GlidePatch_t*)W_CachePatchNum (viewborderlump[BRDR_B],PU_CACHE);
        for (x=0 ; x<baseviewwidth ; x+=8)
    ...
    if (clearlines > basewindowy) {
        patch = (GlidePatch_t*)W_CachePatchNum (viewborderlump[BRDR_L],PU_CACHE);
    ...
    // right edge
    if (clearlines > basewindowy) {
        patch = (GlidePatch_t*)W_CachePatchNum (viewborderlump[BRDR_R],PU_CACHE);
        for (y=0 ; y<baseviewheight && (basewindowy+y < clearlines); y+=8)
    ...
    if (clearlines > basewindowy-8)
        HWR_DrawPatch ((GlidePatch_t*)W_CachePatchNum (viewborderlump[BRDR_TL],PU_CACHE),
                       basewindowx-8,
                       basewindowy-8,0);

    if (clearlines > basewindowy-8)
        HWR_DrawPatch ((GlidePatch_t*)W_CachePatchNum (viewborderlump[BRDR_TR],PU_CACHE),
                       basewindowx+baseviewwidth,
                       basewindowy-8,0);

    if (clearlines > basewindowy+baseviewheight)
        HWR_DrawPatch ((GlidePatch_t*)W_CachePatchNum (viewborderlump[BRDR_BL],PU_CACHE),
                       basewindowx-8,
                       basewindowy+baseviewheight,0);

    if (clearlines > basewindowy+baseviewheight)
        HWR_DrawPatch ((GlidePatch_t*)W_CachePatchNum (viewborderlump[BRDR_BR],PU_CACHE),
                       basewindowx+baseviewwidth,
                       basewindowy+baseviewheight,0);
}
...
bool HWR_Screenshot (char *lbmname)
{
    ...
    bufr = (unsigned short*)malloc(vid.width*vid.height*2);
    if (!bufr)
        return false;
    bufw = (byte*)malloc(vid.width*vid.height*3);
    if (!bufw)
    {
        free(bufr);
        return false;
...
void saveTGA(char *file_name, int width, int height, GLRGB *buffer)
{
    ...
    tga_hdr.width = LE_SWAP_SHORT(width);
    tga_hdr.height = LE_SWAP_SHORT(height);
