...
//-----------------------------------------------------------------------------
//
// $Id: hw_cache.c,v 1.39 2003/06/11 04:33:46 ssntails Exp $
//
                                  ...
                                  int   bpp)
{
    ...
    column_t    *patchcol;
    byte         alpha;
    byte        *block = (byte*)mipmap->grInfo.data;

    x1 = originx;
    x2 = x1 + LE_SWAP_SHORT(realpatch->width);
...
/*
    CONS_Printf("patch %dx%d texture %dx%d block %dx%d\n", LE_SWAP_SHORT(realpatch->width),
                                                            LE_SWAP_SHORT(realpatch->height),
                                                            texturewidth,
                                                            textureheight,
...
*/
    ...
    for (block += col*bpp; ncols--; block+=bpp, xfrac+=xfracstep)
    {
        patchcol = (column_t *)((byte *)realpatch
                                + LE_SWAP_LONG(realpatch->columnofs[xfrac>>16]));
        ...
        while (patchcol->topdelta != 0xff)
        {
            ...
            while (count>0)
            {
                ...
                switch (bpp) {
                    case 2 : *((unsigned short*)dest) = LE_SWAP_SHORT((alpha<<8) | texel);       break;
...
static byte *MakeBlock( GlideMipmap_t *grMipmap )
{
    ...
    block = (byte*)Z_Malloc (blocksize*bpp, PU_STATIC, &(grMipmap->grInfo.data));
    ...
    switch (bpp) {
        ...
        case 2:
                ...
                for( i=0; i<blocksize; i++ )
                   *((unsigned short*)block+i) = LE_SWAP_SHORT((0x00 <<8) | HWR_PATCHES_CHROMAKEY_COLORINDEX);
                break;
...
static void HWR_GenerateTexture (int texnum, GlideTexture_t* grtex)
{
    ...
    int         i;
    bool     skyspecial = false; //poor hack for Legacy large skies..
         ...
         i++, patch++)
    {
        ...
        if (skyspecial) {
            //CONS_Printf("sky %d, %d\n",texture->width,LE_SWAP_SHORT(realpatch->width));
            //texture->width = LE_SWAP_SHORT(realpatch->width);
            texture->height = LE_SWAP_SHORT(realpatch->height);
        }
...
void HWR_MakePatch (patch_t* patch, GlidePatch_t* grPatch, GlideMipmap_t *grMipmap)
{
    ...
    if(grMipmap->width==0)
    {
        ...
        grPatch->width = LE_SWAP_SHORT(patch->width);
        grPatch->height = LE_SWAP_SHORT(patch->height);
        grPatch->leftoffset = LE_SWAP_SHORT(patch->leftoffset);
        grPatch->topoffset = LE_SWAP_SHORT(patch->topoffset);

        // find the good 3dfx size (boring spec)
        HWR_ResizeBlock (LE_SWAP_SHORT(patch->width), LE_SWAP_SHORT(patch->height), &grMipmap->grInfo);
...
/*	else if(cv_voodoocompatibility.value) // Only scales down textures that exceed 256x256.
	{
        ...
        newwidth  = min( LE_SWAP_SHORT(patch->width) , blockwidth );
        newheight = min( LE_SWAP_SHORT(patch->height), blockheight);
...
void HWR_PrepLevelCache (int numtextures)
{
    ...
    gr_textures = (GlideTexture_t*)malloc (sizeof(GlideTexture_t) * numtextures);
    if (!gr_textures)
...
static void HWR_CacheFlat (GlideMipmap_t* grMipmap, int flatlumpnum)
{
    ...
    block = (byte*)Z_Malloc (W_LumpLength(flatlumpnum),
                      PU_HWRCACHE,
...
static void HWR_LoadMappedPatch(GlideMipmap_t *grmip, GlidePatch_t *gpatch)
{
    if( !grmip->downloaded &&
        !grmip->grInfo.data )
    {
        patch_t *patch = (patch_t*)W_CacheLumpNum(gpatch->patchlump, PU_STATIC);
...
void HWR_GetPatch( GlidePatch_t* gpatch )
{
         ...
         !gpatch->mipmap.grInfo.data )
    {
        ...
        patch_t *ptr = (patch_t*)W_CacheLumpNum(gpatch->patchlump, PU_STATIC);
...
void HWR_GetMappedPatch(GlidePatch_t* gpatch, byte *colormap)
{
    ...
    newmip = (GlideMipmap_t*)malloc(sizeof(GlideMipmap_t));
                                ...
                                int   bpp)
{
    ...
    stepy = ((int)LE_SWAP_SHORT(pic->height)<<16)/blockheight;
    stepx = ((int)LE_SWAP_SHORT(pic->width )<<16)/blockwidth;
    ...
    for( j=0 ;j<blockheight;j++)
    {
        ...
        src = &pic->data[(posy>>16)*LE_SWAP_SHORT(pic->width)*picbpp];
        for( i=0 ;i<blockwidth;i++)
        {
...
GlidePatch_t *HWR_GetPic( int lumpnum )
{
        ...
        && !grpatch->mipmap.grInfo.data )
    {
        ...
        if( grpatch->mipmap.flags & TF_RAWASPIC )
        {
            ...
            pic = (pic_t*)W_CacheRawAsPic( lumpnum, grpatch->width, grpatch->height, PU_STATIC );
            len = W_LumpLength(lumpnum);
        }
        else
        {
            pic = (pic_t*)W_CacheLumpNum( lumpnum, PU_STATIC );
            grpatch->width = LE_SWAP_SHORT(pic->width);
            grpatch->height = LE_SWAP_SHORT(pic->height);
...
/*		else if(cv_voodoocompatibility.value) // Only scales down textures that exceed 256x256.
		{
			// no rounddown, do not size up patches, so they don't look 'scaled'
            newwidth  = min(LE_SWAP_SHORT(pic->width ),blockwidth);
            newheight = min(LE_SWAP_SHORT(pic->height),blockheight);
