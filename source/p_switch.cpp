...
void P_InitSwitchList(void)
{
  ...
  if(W_CheckNumForName("SWITCHES") != -1)
  {
    ...
    for (i=0;alphSwitchList[i].episode!=0;i++)
        alphSwitchList[i].episode = LE_SWAP_SHORT(alphSwitchList[i].episode);
  }
  else 
  ...
  if(NULL == switchlist)
      switchlist = (int *)malloc(sizeof(*switchlist));
                           ...
                           int           side )
{
    ...
    switch (line->special)
    {
      ...
      case 7:
        // Build Stairs
        if (EV_BuildStairs(line,(gamemode == heretic) ? (stair_e)(8*FRACUNIT) : build8))
            P_ChangeSwitchTexture(line,0);
        break;
