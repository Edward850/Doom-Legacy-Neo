...
static void next_token()
{
    ...
    if (num_tokens > 1 && *rover == '(' && tokentype[num_tokens - 2] == tname)
        tokentype[num_tokens - 2] = tfunction;
    ...
    {
        tt = tstring;
        if (tokentype[num_tokens - 2] == tstring)
            num_tokens--;       // join strings
        rover++;
    }
    else
    {
        tt = isop(*rover) ? toperator      : isnum(*rover) ? tnumber : tname;
    }
}
...
svalue_t evaluate_expression(int start, int stop)
{
    ...
    if (tokentype[start] == toperator && tokentype[stop] == toperator)
