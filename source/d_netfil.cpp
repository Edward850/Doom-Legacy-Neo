...
//-----------------------------------------------------------------------------
//
// $Id: d_netfil.c,v 1.25 2003/05/04 04:28:33 sburke Exp $
//
...
found:
    ...
    p->position = LE_SWAP_LONG(p->position);
    p->size     = LE_SWAP_SHORT(size);
    p->fileid   = f->fileid;
...
static int time=0;
	...
	netbuffer->u.filetxpak.position	= LE_SWAP_LONG(netbuffer->u.filetxpak.position);
	netbuffer->u.filetxpak.size	= LE_SWAP_SHORT(netbuffer->u.filetxpak.size);
