          ...
          switch(current->type)
            ...
            case IVT_STRING:
              *(char**)current->variable         // +5 for safety
                = (char*)Z_Malloc(strlen(equals)+5, PU_LEVEL, NULL);
...
void P_ParseScriptLine(char *line)
{
  if(levelscript.data[0] == 0)
  {
    ...
    levelscript.data = (char*)Z_Malloc(maxscriptsize, PU_LEVEL, 0);
...
void P_ParseInterText(char *line)
{
  ...
  {
    info_intertext = (char*)Z_Malloc(maxscriptsize, PU_STATIC, 0);
...
bool default_weaponowned[NUMWEAPONS];
...
void P_LoadLevelInfo(int lumpnum)
{
  ...
  readline = (char*)Z_Malloc(lumpsize + 1, PU_STATIC, 0);
  readline[0] = '\0';

  if(lumpsize > 0)
  {
    rover = lump = (char*)W_CacheLumpNum(lumpnum, PU_STATIC);
