...
bool G_InventoryResponder(player_t *ply, int gc[num_gamecontrols][2], event_t *ev)
{
    if( !inventory )
        return false;
    switch (ev->type) {
...
void G_BuildTiccmd (ticcmd_t* cmd, int realtics)
{
    ...
    static bool  keyboard_look;      // true if lookup/down using keyboard
...
void G_DoLoadLevel (bool resetplayer)
{
    ...
    if (wipegamestate == GS_LEVEL)
        wipegamestate = GS_FORCED;             // force a wipe
...
void G_PlayerFinishLevel (int player)
{
    ...
    if(p->chickenTics)
    {
        p->readyweapon = (weapontype_t)p->mo->special1; // Restore weapon
        p->chickenTics = 0;
    }
    p->rain1 = NULL;
    p->rain2 = NULL;
...
bool G_Downgrade(int version)
{
    ...
    else
    {
        ...
        states[S_ROCKET].action.acv     = (actionf_v)(A_SmokeTrailer);

        // smoke trails behind the skull heads
        states[S_SKULL_ATK3].action.acv = (actionf_v)(A_SmokeTrailer);
        states[S_SKULL_ATK4].action.acv = (actionf_v)(A_SmokeTrailer);
    }
...
//
    ...
    if ((i=W_CheckNumForName(defdemoname)) == -1)
    {
        ...
        demobuffer = demo_p = (byte*)W_CacheLumpNum (i, PU_STATIC);
