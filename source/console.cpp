...
//-----------------------------------------------------------------------------
//
// $Id: console.c,v 1.23 2003/08/11 13:50:03 hurdler Exp $
//
...
void CONS_Printf (char *fmt, ...)
{
    ...
    if (con_startup)
    {
#if 0 //defined( __WIN32__) || defined( __OS2__) 
...
static void CON_DrawBackpic (pic_t *pic, int startx, int destwidth)
{
    ...
    for (y=0 ; y<con_curlines ; y++, dest += vid.width)
    {
        ...
        v = LE_SWAP_SHORT(pic->height) - ((con_curlines - y)*(BASEVIDHEIGHT-1)/vid.height) - 1;

        src = pic->data + v*LE_SWAP_SHORT(pic->width);

        // in case of the console backpic, simplify
        if (LE_SWAP_SHORT(pic->width) == destwidth)
            memcpy (dest, src, destwidth);
        else
        {
            // scale pic to screen width
            frac = 0;
            fracstep = (LE_SWAP_SHORT(pic->width)<<16)/destwidth;
