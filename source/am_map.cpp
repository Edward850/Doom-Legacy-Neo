...
//-----------------------------------------------------------------------------
//
// $Id: am_map.c,v 1.12 2001/08/20 20:40:39 metzgermeister Exp $
//
...
bool         am_recalc = false;     //added:05-02-98:true when screen size
                                       //               changes
...
static void AM_loadPics(void)
{
    ...
    for (i=0;i<10;i++)
    {
        I_sprintf(namebuf, sizeof(namebuf), "AMMNUM%d", i);
    ...
    else if (ev->type == ev_keydown)
    {
        ...
        switch(ev->data1)
        {
          ...
          case AM_GRIDKEY:
            ...
            break;
          case AM_MARKKEY:
            I_sprintf(buffer, sizeof(buffer), "%s %d", AMSTR_MARKEDSPOT, markpointnum);
            plr->message = buffer;
            AM_addMark();
            break;
...
void AM_drawMarks(void)
{
    ...
    for (i=0;i<AM_NUMMARKPOINTS;i++)
    {
        if (markpoints[i].x != -1)
        {
            //      w = LE_SWAP_SHORT(marknums[i]->width);
            //      h = LE_SWAP_SHORT(marknums[i]->height);
