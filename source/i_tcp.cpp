...
//-----------------------------------------------------------------------------
//
// $Id: i_tcp.c,v 1.39 2003/05/04 02:31:53 sburke Exp $
//
...
#if defined(BUILD_IPX) && !defined(SOLARIS)
...
static bool  nodeconnected[MAXNETNODES+1];
static bool  ipx;
...
#if defined(BUILD_IPX) && !defined(SOLARIS)
bool IPX_cmpaddr(mysockaddr_t *a,mysockaddr_t *b)
{
#ifdef LINUX
#ifdef FREEBSD
...
bool UDP_cmpaddr(mysockaddr_t *a,mysockaddr_t *b)
{
    return (a->ip.sin_addr.s_addr == b->ip.sin_addr.s_addr && a->ip.sin_port == b->ip.sin_port);
}

bool (*SOCK_cmpaddr) (mysockaddr_t *a,mysockaddr_t *b);
