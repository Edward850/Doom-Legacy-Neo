...
//-----------------------------------------------------------------------------
//
// $Id: b_search.c,v 1.3 2002/09/28 06:53:11 tonyd Exp $
//
...
{
	LinkedList_t* ll = (LinkedList_t*)Z_Malloc (sizeof(LinkedList_t), PU_STATIC, NULL);
...
{
	PriorityQ_t* pq = (PriorityQ_t*)Z_Malloc (sizeof(PriorityQ_t), PU_LEVEL, NULL);
	pq->maxSize = mx;
	pq->currentSize = 0;
	pq->heapArray = (SearchNode_t**)Z_Malloc (((sizeof(SearchNode_t*))*mx), PU_LEVEL, NULL);
...
bool B_FindNextNode(player_t* p)
{
	bool		found = false;
	...
	if (closestnode)// || P_AproxDistance(p->mo->x - closestnode->x, p->mo->y - closestnode->y) > (BOTNODEGRIDSIZE<<1))	//no nodes can get here
	{
		...
		if (bestNode && (bestNode != closestnode))
		{
			...
			{
				tempNode = (SearchNode_t*)Z_Malloc(sizeof(SearchNode_t),PU_STATIC,0);
