...
//-----------------------------------------------------------------------------
//
// $Id: w_wad.c,v 1.33 2002/07/29 21:52:25 hurdler Exp $
//
...
int W_LoadWadFile (char *filename)
{
    ...
    {   // assume wad file
        ...
        header.numlumps = LE_SWAP_LONG(header.numlumps);
        header.infotableofs = LE_SWAP_LONG(header.infotableofs);
        ...
        for (i=0 ; i<numlumps ; i++,lump_p++, fileinfo++)
        {
            ...
            lump_p->position = LE_SWAP_LONG(fileinfo->filepos);
            lump_p->size     = LE_SWAP_LONG(fileinfo->size);
    ...
    //
    fstat(handle,&bufstat);
    wadfile = (wadfile_t*)Z_Malloc (sizeof (wadfile_t),PU_STATIC,NULL);
    ...
    fhandle = fopen(filenamebuf, "rb");
    {
        ...
        P_HashMD5Init();
        size_t bytesRead;
        while ((bytesRead = fread(fileData, 1, sizeof(fileData), fhandle)) > 0)
        {
            P_HashMD5Data(fileData, bytesRead);
        }
        P_HashMD5Finalize(wadfile->md5sum);
        if( devparm )
    ...
    lumpcache = (void**)Z_Malloc (length,PU_STATIC,NULL);
    ...
    grPatch = (GlidePatch_t*)Z_Malloc (length, PU_HWRPATCHINFO, 0);    //never freed
...
void W_Reload (void)
{
    ...
    read (handle, &header, sizeof(header));
    lumpcount = LE_SWAP_LONG(header.numlumps);
    header.infotableofs = LE_SWAP_LONG(header.infotableofs);
...
void* W_CacheLumpNum ( int lump, int tag )
{
    ...
    if (!lumpcache[llump]) {
        ...
        ptr = (byte*)Z_Malloc (W_LumpLength (lump), tag, &lumpcache[llump]);
...
//
//
patch_t* W_CachePatchName ( char*   name,
                         int     tag )
{
...
pic_t *W_CacheRawAsPic( int lump, int width, int height, int tag)
{
    lumpcache_t*  lumpcache;
    //SoM: 4/8/2000: Don't keep doing oporations to the lump variable!
    int           llump = lump & 0xffff;
    ...
    if (!lumpcache[llump]) 
    {
        ...
        pic = (pic_t*)Z_Malloc (W_LumpLength (lump)+sizeof(pic_t), tag, &lumpcache[llump]);
        W_ReadLumpHeader (lump, pic->data, 0);   // read full
        pic->width = LE_SWAP_SHORT(width);
        pic->height = LE_SWAP_SHORT(height);
        pic->mode = PALETTE;
    }
    else 
        Z_ChangeTag (lumpcache[llump],tag);

    return (pic_t*)lumpcache[llump];
}
...
int W_AddFile (char *filename)
{
    ...
    if (stricmp (filename+strlen(filename)-3, "wad") )
    {
        ...
        singleinfo.size = LE_SWAP_LONG(filelen(handle));
