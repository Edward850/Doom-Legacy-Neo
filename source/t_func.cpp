...
void SF_PlayerSelectedWeapon()
{
	...
	else if(t_argc == 2)
	{
		...
		players[playernum].pendingweapon = (weapontype_t)weaponnum;
...
void SF_Spawn()
{
    ...
    {   //Hurdler: fix the crashing bug of respawning monster
        mapthing_t *mthing;
        mthing = (mapthing_t *)Z_Malloc(sizeof(mapthing_t), PU_LEVEL, NULL);
        mthing->x = x >> FRACBITS;
        mthing->y = y >> FRACBITS;
        mthing->z = z >> FRACBITS;
...
void SF_SpawnMissile()
{
	...
	t_return.type = svt_mobj;
    t_return.value.mobj = P_SpawnMissile(mobj, target, (mobjtype_t)objtype);
}
...
{					//DarkWolf95:December 7, 2003: Change to set only
	statenum_t state, newstate;
	mobj_t	*mo;

	if(t_argc == 1)
	{
		mo = current_script->trigger;
		state = (statenum_t)intvalue(t_argv[0]);
	}

	else if(t_argc == 2)
	{
		mo = MobjForSvalue(t_argv[0]);
		state = (statenum_t)intvalue(t_argv[1]);
	}
...
void SF_FloorHeight()
{
    ...
    if (t_argc > 1)     // > 1: set floorheight
    {
        ...
        while ((i = P_FindSectorFromTag(tagnum, i)) >= 0)
        {
            ...
            if (T_MovePlane(&sectors[i], abs((int)(fixedvalue(t_argv[1]) - sectors[i].floorheight)), fixedvalue(t_argv[1]), crush, 0, fixedvalue(t_argv[1]) > sectors[i].floorheight ? 1 : -1) == crushed)
                returnval = 0;
        }
    }
    else
...
void SF_CeilingHeight()
{
    ...
    if (t_argc > 1)     // > 1: set ceilheight
    {
        ...
        while ((i = P_FindSectorFromTag(tagnum, i)) >= 0)
        {
            ...
            if (T_MovePlane(&sectors[i], abs((int)(fixedvalue(t_argv[1]) - sectors[i].ceilingheight)), fixedvalue(t_argv[1]), crush, 1, fixedvalue(t_argv[1]) > sectors[i].ceilingheight ? 1 : -1) == crushed)
                returnval = 0;
        }
    }
    else
...
void SF_StartSkill()
{
    ...
    G_DeferedInitNew((skill_t)skill, G_BuildMapName(1, 1), false);
}
