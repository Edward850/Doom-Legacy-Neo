                     ...
                     bool       refresh )
{
    ...
    int    w = LE_SWAP_SHORT(n->p[0]->width);
    int    h = LE_SWAP_SHORT(n->p[0]->height);
    int    x = n->x;
                            ...
                            bool               refresh )
{
        ...
        && (*mi->inum!=-1))
    {
        if (mi->oldinum != -1)
        {
            x = mi->x - LE_SWAP_SHORT(mi->p[mi->oldinum]->leftoffset);
            y = mi->y - LE_SWAP_SHORT(mi->p[mi->oldinum]->topoffset);
            w = LE_SWAP_SHORT(mi->p[mi->oldinum]->width);
            h = LE_SWAP_SHORT(mi->p[mi->oldinum]->height);
                           ...
                           bool               refresh )
{
    ...
    {
        x = bi->x - LE_SWAP_SHORT(bi->p->leftoffset);
        y = bi->y - LE_SWAP_SHORT(bi->p->topoffset);
        w = LE_SWAP_SHORT(bi->p->width);
        h = LE_SWAP_SHORT(bi->p->height);
