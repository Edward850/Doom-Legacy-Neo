...
struct ptrChain
{
    ...
    ptrChain* mnext;
    ptrChain* mprev;
    ptrChain(memblock_t* p) : ptr(p), next(nullptr), prev(nullptr), mnext(nullptr), mprev(nullptr) {}
};

static std::unordered_map<size_t, ptrChain*> cacheMap[2];
static ptrChain* globalCacheHead = nullptr;

void Z_AddToCache(memblock_t* ptr, const size_t size, const int tag)
{
    ptrChain* newNode = new ptrChain(ptr);
    std::unordered_map<size_t, ptrChain*>* map = &cacheMap[tag - PU_CACHE];
...
memblock_t* Z_FindFreeBlock(const size_t size, const int tag)
{
    std::unordered_map<size_t, ptrChain*>* map = &cacheMap[tag - PU_CACHE];
    auto it = map->find(size);
    if (it != map->end())
    {
        ...
        memblock_t* block = it->second->ptr;
        ptrChain* current = it->second;
        Z_UnlinkFromCache(current);
        if (current == it->second) // If we're removing the head, move the head pointer
        {
...
memblock_t* Z_GetFirstGlobalBlock()
{
