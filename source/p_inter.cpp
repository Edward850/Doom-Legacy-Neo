...
void P_SetMessage(player_t *player, char *message, bool ultmsg)
{
...
void VerifFavoritWeapon (player_t *player)
{
    ...
    if (newweapon != player->readyweapon)
        player->pendingweapon = (weapontype_t)newweapon;
}
...
// P_GiveCard
//
static bool P_GiveCard ( player_t*     player,
                            card_t        card )
{
...
bool P_TouchWhirlwind(mobj_t *target)
{
    int randVal;
...
//
bool P_DamageMobj ( mobj_t*   target,
                       mobj_t*   inflictor,
                       mobj_t*   source,
                       int       damage )
{
