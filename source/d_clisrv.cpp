...
//-----------------------------------------------------------------------------
//
// $Id: d_clisrv.c,v 1.45 2003/11/22 00:22:08 darkwolf95 Exp $
//
...
bool         server = true;           // true or false but !server=client
...
static bool CL_SendJoin()
{
    ...
    netbuffer->u.clientcfg.subversion = LE_SWAP_LONG(SUBVERSION);
...
static void SV_SendServerInfo(int node, tic_t time)
{
    ...
    netbuffer->u.serverinfo.subversion = LE_SWAP_LONG(SUBVERSION);
    // return back the time value so client can compute there ping
    netbuffer->u.serverinfo.time = LE_SWAP_LONG(time);
...
static bool SV_SendServerConfig(int node)
{
    int   i,playermask=0;
    byte  *p;
    ...
    netbuffer->u.servercfg.subversion      = LE_SWAP_LONG(SUBVERSION);

    netbuffer->u.servercfg.serverplayer    = serverplayer;
    netbuffer->u.servercfg.totalplayernum  = doomcom->numplayers;
    netbuffer->u.servercfg.playerdetected  = LE_SWAP_LONG(playermask);
    netbuffer->u.servercfg.gametic         = LE_SWAP_LONG(gametic);
...
void SendAskInfo( int node )
{
    ...
    netbuffer->u.askinfo.time = LE_SWAP_LONG(I_GetTime());
...
void Command_connect(void)
{
    ...
    if( I_stricmp(COM_Argv(1),"self")==0 )
    {
        servernode = 0;
        server = true;
        // should be but...
    ...
    else
    {
        ...
        if( netgame && I_stricmp(COM_Argv(1),"node")==0 )
...
int nametonum(char *name)
{
    ...
    for(i=0;i<MAXPLAYERS;i++)
        if(playeringame[i] && I_stricmp(player_names[i],name)==0)
            return i;
    ...
    while ( HGetPacket() )
    {
        ...
        if( netbuffer->packettype == PT_CLIENTJOIN && server )
        {
            if(         netbuffer->u.clientcfg.version     != VERSION
                || LE_SWAP_LONG(netbuffer->u.clientcfg.subversion) != SUBVERSION)
        ...
        {
            // compute ping in ms
            netbuffer->u.serverinfo.time = (I_GetTime() - LE_SWAP_LONG(netbuffer->u.serverinfo.time))*1000/TICRATE; 
        ...
        if(!nodeingame[node])
        {
            ...
            switch(netbuffer->packettype) {
                case PT_ASKINFO:
                    ...
                    {
                        SV_SendServerInfo(node, LE_SWAP_LONG(netbuffer->u.askinfo.time));
                        Net_CloseConnection(node);
                    }
                    break;
                ...
                case PT_SERVERCFG :    // positive responce of client join request
                {
                    ...
                    if(!server)
                        maketic = gametic = neededtic = LE_SWAP_LONG(netbuffer->u.servercfg.gametic);
#ifdef CLIENTPREDICTION2
                    localgametic = gametic;
#endif
                    ...
                    for(j=0;j<MAXPLAYERS;j++)
                        playeringame[j]=(LE_SWAP_LONG(netbuffer->u.servercfg.playerdetected) & (1<<j))!=0;
...
// -------------------------------------------- SERVER RECEIVE ----------
            ...
            case PT_NODEKEEPALIVEMIS :
                ...
                if((realstart <= gametic) && (realstart > gametic-BACKUPTICS+1) &&
                   (consistancy[realstart%BACKUPTICS] != LE_SWAP_SHORT(netbuffer->u.clientpak.consistancy))) 
                {
#if 1
                    char buf[3];
...
#endif
                    DEBFILE(va("player %d kicked [%d] %d!=%d\n",netconsole,realstart,consistancy[realstart%BACKUPTICS],LE_SWAP_SHORT(netbuffer->u.clientpak.consistancy)));
...
*/
    ...
    if( gamestate!=GS_NULL )
    {
        ...
        netbuffer->u.clientpak.consistancy = LE_SWAP_SHORT(consistancy[gametic%BACKUPTICS]);
...
static void SV_SendTics (void)
{
    ...
    for(n=1;n<MAXNETNODES;n++)
        if( nodeingame[n] )
        {
            ...
            netbuffer->u.serverpak.numplayers= LE_SWAP_SHORT(doomcom->numplayers);
