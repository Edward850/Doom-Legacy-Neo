...
section_t *new_section(char *brace)
{
  ...
  newsec = (section_t *)Z_Malloc(sizeof(section_t), PU_LEVEL, 0);
  newsec->start = brace;
  ...
  while(rover < end && *rover)
    {
      ...
      if(current_section && tokentype[0] == tfunction)
	{
...
void parse_include(char *lumpname)
{
  ...
  lump = (char *)W_CacheLumpNum(lumpnum, PU_STATIC);
  ...
  temp = lump;
  lump = (char *)Z_Malloc(W_LumpLength(lumpnum) + 10, PU_STATIC, NULL);
