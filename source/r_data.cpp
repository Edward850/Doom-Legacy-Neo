...
//-----------------------------------------------------------------------------
//
// $Id: r_data.c,v 1.31 2003/05/04 04:12:54 sburke Exp $
//
...
byte* R_GenerateTexture(int texnum)
{
    ...
    if (texture->patchcount == 1)
    {
        ...
        for (i = 0; i < texture->width; i++)
            colofs[i] = LE_SWAP_LONG(colofs[i]) + 3;
        ...
        i++, patch++)
    {
        ...
        for (; x < x2; x++)
        {
            patchcol = (column_t*)((byte*)realpatch
                + LE_SWAP_LONG(realpatch->columnofs[x - x1]));
...
{
    return (byte *)W_CacheLumpNum (flatlumpnum, PU_CACHE);
...
void R_LoadTextures (void)
{
    ...
    name[8] = 0;
    pnames = (char *)W_CacheLumpName ("PNAMES", PU_STATIC);
    nummappatches = LE_SWAP_LONG ( *((int *)pnames) );
    name_p = pnames+4;
    ...
    numtextures1 = LE_SWAP_LONG(*maptex);
    ...
    if (W_CheckNumForName ("TEXTURE2") != -1)
    {
        ...
        numtextures2 = LE_SWAP_LONG(*maptex2);
    ...
    textures = (texture_t **)Z_Malloc((int)tablebytes, PU_STATIC, 0);
    ...
    for (i=0 ; i<numtextures ; i++, directory++)
    {
        ...
        texture = textures[i] =
            (texture_t *)Z_Malloc (sizeof(texture_t)
                      + sizeof(texpatch_t)*(LE_SWAP_SHORT(mtexture->patchcount)-1),
                      PU_STATIC, 0);

        texture->width  = LE_SWAP_SHORT(mtexture->width);
        texture->height = LE_SWAP_SHORT(mtexture->height);
        texture->patchcount = LE_SWAP_SHORT(mtexture->patchcount);

        if (texture->width <= 0 || texture->width > 4096 ||
            texture->height <= 0 || texture->height > 4096)
        {
            I_Error("R_LoadTextures: invalid texture size %.8s (%d x %d)",
	...
	// gcc will replace memcpy with two 4-byte read/writes, which will bus error.
        ...
        {
            patch->originx = LE_SWAP_SHORT(mpatch->originx);
            patch->originy = LE_SWAP_SHORT(mpatch->originy);

            patchnum = LE_SWAP_SHORT(mpatch->patch);
...
void R_InitSpriteLumps (void)
{
    ...
    spritewidth = (int *)Z_Malloc (MAXSPRITELUMPS*4, PU_STATIC, 0);
    spriteoffset = (int *)Z_Malloc (MAXSPRITELUMPS*4, PU_STATIC, 0);
    spritetopoffset = (int *)Z_Malloc (MAXSPRITELUMPS*4, PU_STATIC, 0);
    spriteheight = (int *)Z_Malloc (MAXSPRITELUMPS*4, PU_STATIC, 0);
}
...
void R_InitColormaps (void)
{
    ...
    colormaps = (byte *)Z_MallocAlign (W_LumpLength (lump), PU_STATIC, 0, 16);
...
int R_ColormapNumForName(char *name)
{
  ...
  extra_colormaps[num_extra_colormaps].colormap = (byte *)Z_MallocAlign (W_LumpLength (lump), PU_LEVEL, 0, 8);
...
#endif
  {
    colormap_p = (char *)Z_MallocAlign((256 * 34) + 10, PU_LEVEL, 0, 16); // Aligning on 16 bits, NOT 8, keeps it from crashing! SSNTails 12-13-2002
...
void R_Init8to16 (void)
{
    ...
    palette = (byte *)W_CacheLumpName ("PLAYPAL",PU_CACHE);
    ...
    // test a big colormap
    hicolormaps = (short *)Z_Malloc (32768 /**34*/, PU_STATIC, 0);
    for (i=0;i<16384;i++)
         hicolormaps[i] = i<<1;
}
