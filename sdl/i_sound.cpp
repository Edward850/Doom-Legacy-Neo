...
static void *getsfx(const char *sfxname, int *len, int* rate)
{
    ...
    if (lumpsize > 32)
    {
        ...
        // Sample rate header
		uint16_t rate16;
        memcpy(&rate16, &sfx[0x02], sizeof(uint16_t));
		*rate = rate16;

        // Length and padding
        memcpy(&size, &sfx[0x04], sizeof(int32_t));

        if (size > 32 && ((size - 32) - 1) + 0x18 < lumpsize)
        {
            // Strip the padding
            size -= 32;
            loadedsfx = (unsigned char*)Z_Malloc(size, PU_STATIC, 0);
    ...
    if (loadedsfx == NULL)
    {
        loadedsfx = (unsigned char*)Z_Malloc(sizeof(unsigned char), PU_STATIC, 0);
        *loadedsfx = 0;
        *len = 1;
    }
