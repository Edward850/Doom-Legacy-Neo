...
char* VID_GetModeName(int modeNum)
{
    ...
    {
		vidModeName[modeNum] = (char*)malloc(16 * sizeof(char));
...
void I_StartupGraphics(void)
{
	...
	modeList = (SDL_Rect**)malloc(sizeof(SDL_Rect*) * (displayCount + 1));
    if (modeList == NULL)
    {
    ...
    for(int i = 0; i < displayCount; i++) 
    {
        if(checkModeExists(displayModes[i]->w, displayModes[i]->h))
			continue;
        modeList[numVidModes] = (SDL_Rect*)malloc(sizeof(SDL_Rect));
	...
	vidModeName = (char**)malloc(sizeof(char*) * numVidModes);
