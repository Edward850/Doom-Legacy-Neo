...
#ifdef _WIN32
filestatus_t filesearch(char* filename, char* startpath, unsigned char* wantedmd5sum, bool completepath, int maxsearchdepth)
{
    ...
    if ((hFind = FindFirstFile(filename, &FindFileData)) != INVALID_HANDLE_VALUE)
    {
        do 
        {
            switch (checkfilemd5(FindFileData.cFileName, wantedmd5sum))
            {
            case FS_FOUND:
